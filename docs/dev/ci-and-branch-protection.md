...
- **`file:` dependency guard** — `check.yml` fails fast if `editors/vscode/package.json` carries a `file:`/portal dep (left over from `make use-local`); run `make use-npm VERSION=X.Y.Z` to pin the released version before pushing. Name the version: bare `make use-npm` (`make un`) installs `latest` and writes the literal spec `npm:latest`, which satisfies every guard while recording nothing — the manifest then states no version and `yarn.lock` is the only place the answer lives. The `bump-mthds-ui` skill walks the whole move, changelog included. The stricter guard on that one dependency is `make check-no-local-deps` inside `make check`, and `.githooks/pre-commit` runs the same `scripts/check-mthds-ui-spec.sh`: it admits `npm:` and a sprint pin written by the workspace's `wt pin` as `github:<owner>/<repo>#<40-hex sha>`, and refuses everything else — a `portal:` or `file:` link above all, and a `github:` source at a branch, a tag or an abbreviated SHA, because those move under the lockfile. A sprint pin passes the gate so the branch carrying it can run CI, and it is collapsed back onto `npm:` with `wt unpin` before that branch merges.
