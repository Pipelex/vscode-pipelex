...
2. **Rust parity** (`crates/pipelex-cli/tests/parity.rs`) — the in-process library output must match the shipped `plxt` binary (`plxt fmt -` / `plxt lint -`) on the `test-data/mthds` fixtures and the vendored MTHDS Test Corpus in `test-data/mthds-corpus`, so the two can't drift. It lives in `pipelex-cli` (not `pipelex-py`) because only the bin-owning crate gets `CARGO_BIN_EXE_plxt`; `pipelex-py` is a pure-Rust dev-dependency there.
