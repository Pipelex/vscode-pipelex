...
A backend failure never blanks the **method graph view** either: the static graph stays on screen and the toolbar widget flips to `error` with the failure as its lead issue (CLI missing or too old, no API key, API unreachable, API key rejected, an API error, or an unexpected error). A declined send is the one exception: the user chose not to validate, so no validator was asked, and the widget reads `unvalidated` ("Not validated") with the decline explained as its first row. Actionable failures still raise the toasts described above — including **Set API Key** / **Get an API Key** for the no-key and auth cases — and the next save (or the message views' **Retry**, for pre-graph failures like unreadable bundle files) re-runs the analysis, so a transient failure (server still starting, a network blip, a just-installed CLI, a key just set) recovers without reopening the panel.
