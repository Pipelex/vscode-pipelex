...
pub async fn run_cli(env: JsValue, args: JsValue) -> Result<(), JsError> {
    ...
    let cli = match PlxtArgs::try_parse_from(args) {
        Ok(v) => v,
        Err(error) => {
...
#[cfg(feature = "lsp")]
#[wasm_bindgen]
pub fn create_lsp(env: JsValue, lsp_interface: JsValue) -> lsp::PipelexWasmLsp {
    ...
    // Use taplo_lsp directly (not pipelex_lsp) since we've already wrapped
    // the environment with MthdsEnvironment. pipelex_lsp::create_world would
    // double-wrap.
    lsp::PipelexWasmLsp {
