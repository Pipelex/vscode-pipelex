...
export class MethodGraphPanel implements vscode.Disposable {
    ...
    private setHtml(html: string) {
        ...
        if (isPipelexHtml) {
            ...
            const cspMeta = `<meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'nonce-${nonce}' https://unpkg.com https://cdn.jsdelivr.net; style-src 'nonce-${nonce}' ${cspSource} https://unpkg.com https://fonts.googleapis.com; font-src https://fonts.gstatic.com; img-src ${cspSource} https: data:; connect-src 'none';">`;
