...
### Added
 ...
 - **Validation issues decorate the graph nodes**: Issues that target a pipe now also mark the pipe's nodes in the graph itself (via `@pipelex/mthds-ui` 0.14.0) — a severity ring plus a corner count badge on every invocation, tooltip listing the messages and suggested fixes, worst severity winning per node. The extension fills the target from the validator error's `pipe_code`; static diagnostics carry their own targets. Folding rolls decorations up so a folded controller never hides an error. Clicking a badge opens the widget dropdown, and clicking a dropdown row now pans/flashes the target node in addition to the source jump. Issues without a target (bundle-level parse errors, concept-only errors) stay panel-only.
