...
### Changed
 ...
 - **`pipelex.validation.enabled: false` now disables validation everywhere**: Since the graph no longer needs the backend, disabling validation stops the graph panel's verdict run too — no CLI subprocess, no API upload, no failure toasts on a machine without pipelex — and the panel renders static-only with the widget hidden. (Previously the panel still ran the backend on open and save even with validation disabled.)
...
### Fixed
 ...
 - **In-flight analyzes are cancelled when nobody will read them**: Closing the panel or switching it to a Run Graph (graphspec JSON) view now aborts the panel's in-flight validation run instead of letting the CLI subprocess or API call run to completion for a dropped result; the panel's webview message listener is also disposed with its panel instead of accumulating across open/close cycles.
 - **Dead trusted-HTML surface removed**: `BackendError.detailHtml` (pre-escaped HTML for the deleted full-page auth message) lost its only consumer in the static-first flow and is gone, along with its producer — auth guidance now lives in the plain-text message and the toast's one-click remedies.
