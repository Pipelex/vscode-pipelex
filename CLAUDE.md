...
When modifying graph rendering, always consult `../pipelex/pipelex/graph/reactflow/` to understand the ViewSpec data model (node types, edge types, metadata) that the extension must consume. Changes to `viewspec.py` or `viewspec_transformer.py` in pipelex directly affect the JSON the extension receives.
