...
The extension includes a webview panel that renders method/pipe graphs using ReactFlow. Since the static-first flow, the **GraphSpec is built statically in the extension host** by `@pipelex/mthds-ui`'s static-graph module (`buildStaticGraphSpecFromToml` over the bundle's raw `.mthds` text, primary file first by mthds-ui's shared `orderMthdsSources` rule) and rendered immediately — the configured validation backend (the Pipelex API by default, on `https://api.pipelex.com`, or the `pipelex-agent` CLI when `pipelex.backend` is `cli`) runs only in the background for the **verdict** (the CLI is never invoked with `--view`), which drives a validation widget in the graph toolbar (states: validating / valid / invalid / error / unvalidated; hidden for graphspec-json views). `unvalidated` is what a static graph is until a verdict lands, so it is the state whenever nothing validates the method — `pipelex.validation.enabled` off (no backend run at all), a declined remote send, or an on-save validation skipped because another tool reported errors — and never `error`, which means a validator was asked and failed (a missing API key or CLI included). An invalid bundle keeps its (static) graph on screen — the errors live in the widget's dropdown with click-to-navigate; full-page message views remain only for pre-graph failures (unreadable files, missing webview assets, invalid graphspec JSON). Issues that target a pipe also decorate the graph nodes (severity ring + count badge, fold roll-up, badge↔panel navigation): the rendering lives in `@pipelex/mthds-ui`; the extension only fills each issue's `pipeRef`/`nodeId` target — targeting is **domain-qualified** (`domain_code.pipe_code`, never a bare code): `validationErrorsToIssues` builds the ref from the validator error's `domain_code` + `pipe_code`, qualifies a bare `pipe_code` through the static graphspec's `pipe_registry` only when exactly one domain declares that code (ambiguous codes stay panel-only — never guess), and static diagnostics get their targets from the mthds-ui mapper (`Diagnostic.domain_code`). The same domain constrains static-issue navigation and the declaration-scan fallback in `crossFileDiagnostics`/`bundleResolution`. See `docs/features/method-graph.md` for the full flow, the per-state issue-list policy, and the `setValidationStatus` message.
...
**Persisting the in-graph toggle.** The in-graph theme button is itself a control over `pipelex.graph.theme` — there is one source of truth, not a second hidden store. GraphViewer reports every mode change through its `onThemeChange(mode, resolvedTheme)` callback; `adapter.ts` forwards only a genuine *mode* change (deduped against the host-seeded `config.theme`, so an environment-driven `'system'` re-resolve is not mistaken for a user pick) as a `{ type: 'themeModeChanged', mode }` message. `methodGraphPanel.persistThemeMode` maps the renderer mode onto the setting enum (`'system'`→`auto`, else verbatim) and writes it via `config.update`. **The writer must mirror the reader's scope.** `resolveGraphConfig` reads `pipelex.graph.theme` through `getConfiguration('pipelex')` with **no resource**, and the setting is contributed with **`scope: "window"`** — as are `graph.edgeType` and `graph.toolbarPosition`, i.e. every setting `resolveGraphConfig` reads unscoped (`direction`/`showControllers`/`foldMode` are read *with* the URI and stay `resource`; a scope-invariant test in `settingScopes.test.ts` guards this pairing). `window` scope is settable at user/workspace level but **not** per folder, so a `workspaceFolderValue` can never exist — the unscoped read effectively resolves `workspaceValue ?? globalValue` (the `workspaceFolderValue ?? …` term in the theme inspect is consequently always-undefined dead code, kept only for symmetry). `persistThemeMode` therefore also uses the unscoped accessor and targets `Workspace` (when a workspace value already exists, so the toggle "sticks") or otherwise `Global` — **never `WorkspaceFolder`**: the unscoped reader could not see it, and `window` scope makes VS Code reject a folder-target write outright. The no-op guard compares against the *effective* value **including the contributed `auto` default** (`workspaceValue ?? globalValue ?? defaultValue`): toggling to `'system'` while nothing is explicitly set is a no-op, so it does not pin an explicit `auto` that would clobber a `pipelex.toml` `style.theme`. A *non-default* toggle (dark/light, or system over an existing explicit value) does write an explicit `pipelex.graph.theme`, which then overrides any toml `style.theme` — intended, since it's a deliberate user action. The only config-change listener for the graph is scoped to `pipelex.graph.toolbarPosition` (see "Live update on settings change" above); it does **not** react to `pipelex.graph.theme`, so persisting a theme toggle never re-renders or resets the live viewport — the value is picked up by `resolveGraphConfig` on the next open / restart.

### Toolbar position

The GraphViewer's built-in floating toolbar (direction toggle, fold/expand, zoom, theme) is anchored via the `pipelex.graph.toolbarPosition` setting (eight anchors: `top-left` … `center-right`, default `top-right`). Unlike `theme`, there is **no** `pipelex.toml` source — pipelex's `reactflow_config` has no toolbar key — so the VS Code setting is the only source. Like the other `resolveGraphConfig`-group settings (`theme`, `edgeType`), it is **`window`-scoped** — read unscoped, so it is settable at user/workspace level but never per folder; declaring it `resource` would advertise a folder override the reader silently ignores. `resolveGraphConfig` reads it (guarded by `isToolbarPosition`, falling back to `top-right` on a malformed value) into `GraphRenderConfig.toolbarPosition`; `methodGraphPanel.sendGraphspecToWebview` forwards it on the `setData` `config` payload, and GraphViewer reads `config.toolbarPosition` reactively on every render. The toolbar has no in-graph "move me" control, so nothing is persisted back (contrast the theme toggle above).

**Live update on settings change.** `methodGraphPanel` registers a `vscode.workspace.onDidChangeConfiguration` listener scoped (via `affectsConfiguration('pipelex.graph.toolbarPosition')`) to *only* that key — so it never fires for theme writes and leaves the theme-persistence reasoning below untouched. When the setting changes and a graph is live (`panel` open + `webviewReady`), `onToolbarPositionChanged` re-resolves through `resolveGraphConfig` (so the live path applies the identical guard + default as the initial send) and posts a lightweight `{ type: 'setToolbarPosition', toolbarPosition }` message. The adapter updates only `config.toolbarPosition` and re-renders; because GraphViewer resolves the anchor reactively, the toolbar moves immediately without re-running analysis or resetting the viewport — exactly the `setSystemTheme` pattern. Other graph settings (`edgeType`/`direction`/`foldMode`) still have no live listener and take effect on the next open/refresh.
