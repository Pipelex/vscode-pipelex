...
The procedure is the workspace release play, [`docs/workspace/releasing.md`](../../../../docs/workspace/releasing.md) at the workspace root — `../docs/workspace/releasing.md` from this repo's own root, which resolves the same from the main checkout and from any worktree. Read it first, then run it with what follows. The repo key is `vscode-pipelex`, the base is `dev`, and the pull request targets `main`: a `release/*` branch is the only branch here that does, and a release pull request opened into `dev` by mistake lands the version bump without publishing anything. The release worktree is `_vscode-pipelex--release`, made with `wt add vscode-pipelex release --branch release/vX.Y.Z`, and the `vX.Y.Z` in that branch name is the extension's new version — falling back, when the extension was not bumped, to the CLI's, then the library's, then `@pipelex/tools-wasm`'s.
...
- **`main`'s ruleset is strict, and it accepts only a merge commit.** Strict means the release branch has to be **up to date with `main`** before GitHub will merge it, so a release cut while `main` had moved must take `main` in first — `dev`'s ruleset is non-strict and never asks this, which is why it only ever bites here. The merge method is not a choice either: `main` allows `merge` alone, so `--squash` is refused rather than honoured. Neither is worked around with `--admin`. See `docs/dev/ci-and-branch-protection.md`.
...
- **`@pipelex/tools-wasm` has one downstream consumer, the Pipelex plugin's post-edit hook, and a release that publishes it files the rest of that chain.** The hook lints and formats every `.mthds` edit a builder makes with the engine and blocks the edit on any diagnostic, so the MTHDS schema builders are held to is the one frozen into the tools-wasm the hook embeds. The hook bundle is built in `pipelex-sdk-js` by `npm run build:hook`, where `@pipelex/tools-wasm` is a devDependency pinned to an exact version, and vendored into `pipelex-plugins` by `make vendor-hook`. A new engine therefore reaches builders only after a pin bump in one repo and a re-vendor and plugin release in another, and neither repo notices the publish: v0.17.0 published tools-wasm 0.3.0 with a refreshed schema while the released hook kept 0.1.0 and went on refusing the expanded input-slot form. So when the bump question has moved `js/tools-wasm/package.json`, file both follow-ups as soon as the play's step 3 has the release item, each blocked by what it waits for:

  ```bash
  ledger new --owner pipelex-sdk-js --type task --title "Pin @pipelex/tools-wasm X.Y.Z and rebuild the hook bundle" --gist "hook engine tools-wasm X.Y.Z" --blocked-by <release item> --discovered-from <release item>
  ledger new --owner pipelex-plugins --type task --title "Re-vendor the hook bundle on @pipelex/tools-wasm X.Y.Z and release the plugin" --gist "re-vendor hook, tools-wasm X.Y.Z" --blocked-by <the pipelex-sdk-js item> --discovered-from <release item>
  ```

  Every tools-wasm publish arms them, not only a schema refresh, because the plugin's own release gate, `make check-hook-fresh`, refuses a vendored bundle whose engine is behind npm's latest: any publish is a re-vendor the next plugin release owes. A schema refresh is the urgent case, since until the plugin ships the hook blocks forms the standard and the hosted validator accept, so file both with `--severity high` then and name the newly accepted forms in their bodies (`--body-from <file>`). Name both items in the release summary, so the person landing the release knows the chain goes on.
