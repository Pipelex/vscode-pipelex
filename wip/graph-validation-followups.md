...
Static-issue navigation reconstructs targets from `pipe.<code>` contexts without domain/file identity, and node decorations target by bare `pipeCode` — in a bundle where several domains declare the same pipe code, clicking an issue can open the first matching declaration and badges mark every same-code node. The validator is NOT the limiting layer: the wire already carries `domain_code` alongside `pipe_code` (mirrored in the extension's `ValidationErrorItem`), and pipe execution is unambiguous too (GraphSpec node ids and `pipe_registry` keys are domain-qualified). The domain is dropped downstream, in two places: the extension's mappers (`validationErrorsToIssues` copies only `pipe_code`; static-target resolution scans `[pipe.<code>]` headers first-match across files) and mthds-ui (`ValidationIssue` has no domain field, and `graphValidation.ts` matches nodes on bare `pipe_code`, ignoring the node's `domain_code`). Fix decided: use domain-qualified pipe refs (`domain_code.pipe_code`), as the Pipelex runtime does — full implementation brief with pipelex canon, receipts, design, and checkpoints in `wip/pipe-ref-targeting-brief.md`.
