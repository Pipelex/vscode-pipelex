...
### Added
 - **`@pipelex/tools-wasm` npm package**: New lean WASM binding exposing MTHDS `lintMthds`/`formatMthds` to Node consumers — fully offline (embedded MTHDS schema, no HTTP, no filesystem, no config discovery) and small enough to vendor inside a plugin hook bundle, unlike `@pipelex/lsp` which carries the whole language server. It emits the same `Diagnostic` wire shape as `pipelex-tools-py` and the Pipelex API's `/v1/lint`/`/v1/format`, with format options taking the same snake_case passthrough keys as `plxt fmt -o key=value`. Built from a new `crates/pipelex-tools-wasm` crate over the shared engine, packaged in `js/tools-wasm` (vitest corpus-snapshot and wire-shape tests run against the built bundle), wired into `make test`/`make check`, and published manually via `publish-tools-wasm.sh`. (@pipelex/tools-wasm 0.1.0)
 - **Shared lint/format engine hoisted into `pipelex-common`**: The MTHDS lint/format implementation (`Diagnostic` wire structs, staged lint core, canonical-style format) moved from `crates/pipelex-py` into a new `pipelex_common::tools` module behind a `tools` cargo feature; `pipelex-py` is now a thin PyO3 binding over it. Zero behavior change for `pipelex-tools-py` (guarded by the parity corpus and wheel smoke tests). The lint core gained an offline wrapper (`lint_mthds_offline`, capability-less environment) that compiles on every target and is unit-tested to produce identical diagnostics to the native path — this is what the WASM binding calls.
 - **Documentation**: Added `docs/dev/mthds-engine-bindings.md` describing the multi-binding engine (plxt CLI / `pipelex-tools-py` / `@pipelex/lsp` / `@pipelex/tools-wasm` — one impl, several bindings, parity by construction). Rewrote `js/lsp/README.md` (the `@pipelex/lsp` npm page), which still carried upstream `@taplo/cli` boilerplate, to describe the actual package: the TOML/MTHDS language server in WASM, its usage, and when to prefer `@pipelex/tools-wasm`; takes effect on npm at the next `@pipelex/lsp` publish.
...
### Changed
 ...
 - **Native concept hovers updated for `YesNo` and `Date`:** The LSP now recognizes the current native concept set, including `YesNo`, `Date`, `SearchResult`, and `Composite`, and no longer treats removed `ImgGenPrompt` as native.

### Fixed
 - **Upstream `taplo` + `taplo-lsp`: clippy 1.97 clean:** CI runs unpinned stable Rust, and Rust 1.97's clippy added lints that fired on upstream code, breaking the `make check` gate. Mechanical, behavior-preserving fixes: redundant `&` in the parser `Error` `Display` impl (`taplo/src/parser/mod.rs`), manual `Option::filter` patterns in the hover/completion/rename handlers, and `match`-to-`?` in `taplo-lsp/src/query.rs`.
 - **Upstream `taplo-common`: no-TLS `get_certs` fallback compiles again:** The `cfg(not(any(native-tls, rustls-tls)))` fallback of `get_certs` in `crates/taplo-common/src/util.rs` took `OsString` by value while the shared call site passes a reference, so any reqwest-without-TLS build failed to compile. It now takes `&OsString` like the TLS variant. No behavior change in any previously-buildable feature combination.
