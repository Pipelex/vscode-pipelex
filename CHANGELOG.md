...
- **Every corpus entry must also lint clean.** `crates/pipelex-cli/tests/corpus.rs` requires each `.mthds` file under `test-data/mthds-corpus/` to produce zero diagnostics against the embedded MTHDS schema, which is the check that makes the vendoring pay for itself: `pipelex` adds a pipe kind, the `mthds.schema.json` vendored here goes stale, the re-sync lands, and the build now goes red in that same commit instead of staying green. It sweeps the corpus tree only — `test-data/mthds/` holds negative fixtures whose whole purpose is to be rejected — and fails if it discovers nothing, so a moved or emptied corpus cannot pass vacuously. Every entry vendored today is structurally clean, so cleanliness is asserted unconditionally — and the obvious way to keep it that way is wrong, which is worth saying before someone reaches for it: an entry's `validity` is a *semantic* declaration about what the `pipelex` runtime makes of the bundle, while this gate is structural, so the two axes cross rather than nest. Measured against the invalid entries queued upstream, most lint clean here and a few do not, and those few are the corpus's only probes for staleness in the *permissive* direction — a vendored schema that quietly stopped rejecting an unknown pipe type. Skipping them would throw that away. `wip/corpus-conformance-gap.md` carries the measurement and what a real fix would need. What stays out of reach here is semantic conformance: linting is TOML syntax, DOM validation and JSON Schema, and never resolves references, so a bundle naming a concept that exists nowhere still lints clean, and agreement with an entry's declared `expected_error` needs the `pipelex` runtime rather than this toolchain. That remaining gap is recorded in `wip/corpus-conformance-gap.md`.
