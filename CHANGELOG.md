...
- **`@pipelex/mthds-ui` moves off 0.17.0, and the graph's data viewer goes with it.** The target is the first mthds-ui release carrying the static-graph builder's own expanded input-slot support (mthds-ui #79) — the editor-side work above is half a feature without it, since a slot written `notes = { concept = "Text" }` was unparseable by `buildStaticGraphSpecFromToml` and vanished from the built graph along with its edges. Until that release exists this repo builds against a portal-linked `../mthds-ui` (`make use-local`); `make check` and the `.githooks/pre-commit` hook both refuse a non-npm spec, so the pin has to be set with `make use-npm VERSION=…` before anything merges.
