...
- **The API version gate is now a capability gate** (`ApiVersionGate` → `ApiCapabilityGate`). It asks "does this server do X?" rather than "how new is this server?". A requirement can be satisfied by an advertised `extensions` token — the MTHDS Protocol's own feature-detection hint, and the mechanism this should key on as the hosted plane grows a capability vocabulary — falling back to per-implementation version floors only where one is declared. Anything the gate does not understand passes: the real guarantee is downstream, where an `is_valid: false` verdict arriving without `validation_errors[]` is already rejected with its own remedy, so a missed warning still fails safe while a false alarm sends someone chasing a deployment problem that does not exist. Every verdict is logged to the output channel with the reason that produced it.
