...
### Changed
 ...
 - **Method graph theme follows the editor:** The graph now opens in the palette matching the active VS Code color theme by default. The new `pipelex.graph.theme` setting (`auto`/`dark`/`light`) pins it, and the in-graph theme button still toggles live. A `pipelex.toml` `style.theme` pin is also honored — only an *explicitly set* `pipelex.graph.theme` overrides it, so the contributed `auto` default no longer silently clobbers the toml value.
