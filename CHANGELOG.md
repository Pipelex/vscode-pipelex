...
- **`@pipelex/mthds-ui` moves off 0.17.0, and the graph's data viewer goes with it.** The target is the first mthds-ui release carrying the static-graph builder's own expanded input-slot support (mthds-ui #79) — the editor-side work above is half a feature without it, since a slot written `notes = { concept = "Text" }` was unparseable by `buildStaticGraphSpecFromToml` and vanished from the built graph along with its edges. Until that release exists this repo builds against a portal-linked `../mthds-ui` (`make use-local`); `make check` and the `.githooks/pre-commit` hook both refuse a non-npm spec, so the pin has to be set with `make use-npm VERSION=…` before anything merges.
  ...
  **What a reader sees change: clicking a data node now shows the concept's structure table and no data tab.** That is mthds-ui's deliberate floor — the panel renders a payload only when the host passes both `contracts` (the `pipe_io_contracts` view) and `outputForm` (the `output_form` view), because a descriptor without its schema, or a schema without its descriptor, would leave the panel guessing what the value *is*. For a `.mthds` editor that costs nothing: the graph is built statically from bundle text, so there is no run data to show and the floor is the right rendering. **For `Pipelex: Show GraphSpec JSON` it is a real loss** — a graphspec dumped from an actual run carries its stuff payloads, `StuffViewer` used to render them, and until this extension asks a backend for the two views and forwards them, that panel shows a structure table where it used to show the value. Restoring it takes asking a backend for the two views and forwarding them; the third step it looked like it would take — hand-copying the form kernel's stylesheet into the webview — was deleted by the CSS change below before it was ever written. `docs/features/method-graph.md` records the shape. Until then the kernel rides in `graph.js` and never mounts, so nothing renders unstyled and nothing renders at all.
