...
### Added
 - **Static-first method graph**: The Method Graph panel now renders instantly. The graph is built **statically in the extension** from the bundle's `.mthds` files via `@pipelex/mthds-ui`'s new static-graph module — no `pipelex-agent` round-trip to wait for, and it works even when the method is invalid or pipelex isn't installed at all. Validation still runs, but in the background: the panel fires `pipelex-agent validate` (without `--view` — the graph no longer comes from the CLI) after the graph is already on screen.
